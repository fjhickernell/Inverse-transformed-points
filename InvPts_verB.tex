...
\abstract{\FJH{Lulu or Yiou, please draft me.}}

\section{Introduction}

Professor Kai-Tai Fang and his collaborators have demonstrated the effectiveness of low discrepancy points as space filling designs \cite{FangHic07a, FangEtal19a, FanLiSud06, FanWan94}.  
They have promoted discrepancy as a quality measure for statistical experimental designs to the statistics, science, and engineering communities \cite{FanMa01b, FanMaWin02, FanMuk00, FanMa01a}. 
...
\end{equation}
A number of transformation methods for different distributions can be found in \cite{devroye2006nonuniform} and  \cite[Chap.\ 1]{FanWan94}.
...
\end{equation}
We demonstrate that the answer may be yes or no, depending on how the question is understood.  
...
In the next section of this chapter, we first explain the discrepancy that measures how large $\Ftar - F_\Xdes$ is and motivate it from three different perspectives.  In Section \ref{sec:WhenYes} we give simple condition under which the answer to \eqref{eq:BigQ} is yes.  But, if Section \ref{sec:DoTransformedPts} we show that under give a more practical condition the answer to \eqref{eq:BigQ} is no.  An example illustrates what can go wrong.  Section \ref{sec:CoordEx} provides a coordinate exchange algorithm that improves the discrepancy of an initial candidate design.  Simulation results illustrate the value of this algorithm.  We conclude with a brief discussion.
...
In the remainder of this section we provide three interpretations of the discrepancy, summarized in Table \ref{tab:ThreeInterpTable}.  These results are presented in various places, including \cite{Hic99a,Hic17a}.  One interpretation of discrepancy is the norm of $\nu - \nu_{\Xdes}$.  The second and third interpretations consider the problem of evaluating the mean of a random variable $Y=f(\vX)$, or equivalently a multidimensional integral
...
Typically the computational cost of evaluating $K(\vt,\vx)$ for any $(\vt,\vx) \in \Omega^2$ is $\Order(d)$, where $t$ is a $d$-vector.  Thus, assuming that the integrals above can be evaluated at a cost of $\Order(d)$, the computational cost of evaluating $D(\Xdes,\nu,K)$ is $\Order(dN^2)$.

The formulas for the discrepancy in \eqref{eq:discDef} depend inherently on the choice of the kernel $K$.  That choice is key to answering our question \eqref{eq:BigQ}.  An often used kernel is 
...
Condition \eqref{eq:BigQYesb} may be easily satisfied.  For example, it is automatically satisfied by the inverse cumulative distribution transform \eqref{eq:inverse}.  Condition \eqref{eq:BigQYesa} is simply a matter of definition of the kernel, $K$, but it is definition with consequences.  From the perspective of Section \ref{sec:HilbertMeasures}, changing the kernel from $K_\unif$ to $K$ means changing the definition of the distance between two Dirac measures.  From the perspective of Section \ref{sec:DetermBound}, changing the kernel from $K_\unif$ to $K$ means changing the definition of the Hilbert space of integrands, $f$, in \eqref{eq:integration}.   From the perspective of Section \ref{sec:RandomBound}, changing the kernel from $K_\unif$ to $K$ means changing the definition of the covariance kernel for the integrands, $f$, in \eqref{eq:integration}.
...
\end{equation}
where $K$ is the kernel defined above in \eqref{eq:OrigKernel}.  
...
\centering
{\subfigure[$d=2,N=32$] {\includegraphics[width=5.5cm]{code/boxplot_d2n32.eps}}}
\quad
{\subfigure [$d=3, N=64$]
{\includegraphics[width=5.5cm]{code/boxplot_d3n64.eps}}}
\quad
{\subfigure[$d=4, N=64$] {\includegraphics[width=5.5cm]{code/boxplot_d4n64.eps}}}
\quad
{\subfigure[$d=6, N=128$] {\includegraphics[width=5.5cm]{code/boxplot_d5n128.eps}}}
\quad
{\subfigure[$d=8, N=256$] {\includegraphics[width=5.5cm]{code/boxplot_d8n256.eps}}}
\quad
{\subfigure[$d=10, N=512$] {\includegraphics[width=5.5cm]{code/boxplotall_d10n512.eps}}}
...
\centering
{\subfigure[$d=2,N=32$] {\includegraphics[width=5.5cm]{code/errorboxplot_d2n32.eps}}}
\quad
{\subfigure [$d=3, N=64$]
{\includegraphics[width=5.5cm]{code/errorboxplot_d3n64.eps}}}
\quad
{\subfigure[$d=4, N=64$] {\includegraphics[width=5.5cm]{code/errorboxplot_d4n64.eps}}}
\quad
{\subfigure[$d=6, N=128$] {\includegraphics[width=5.5cm]{code/errorboxplot_d6n128.eps}}}
\quad
{\subfigure[$d=8, N=256$] {\includegraphics[width=5.5cm]{code/errorboxplot_d8n256.eps}}}
\quad
{\subfigure[$d=10, N=512$] {\includegraphics[width=5.5cm]{code/errorboxplot_d10n512.eps}}}
...
\FJH{Lulu, could you please make the references consistent, i.e., all initials and not first names.  Also please use AMS standard abbreviations for the journals.}
