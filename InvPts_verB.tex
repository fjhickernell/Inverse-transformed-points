...
\begin{align}
\nonumber
    D^2(\Xdes,\rho,K)  & = c^d  - \frac 2N \sum_{i=1}^N H(\vx_{i}) + \frac{1}{N^2}  \sum_{i,j=1}^N K(\vx_{i},x_{j}),
    \intertext{where}
    c & = \int_{\tOmega \times \tOmega} \tK(t_k,x_k) \, \tvarrho(t) \tvarrho (x) \, \dif t\dif x, \\
    h(x) & = \int_{\tOmega} \tK_k(t,x)  \, \tvarrho(t) \, \dif t.
    c & = \int_{\Omega_k \times \Omega_k} \tK_k(t_k,x_k) \, \varrho_k(t) \varrho_k (x) \, \dif t\dif x, \\
    H(\vx) = \prod_{k=1}^d [1+ h(x_{k})]
    h(x) & = \int_{\Omega_k} \tK_k(t,x)  \, \varrho_k(t) \, \dif t.
\end{align}
