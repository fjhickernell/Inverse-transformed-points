...
\begin{align}
\nonumber
    D^2(\Xdes,\rho,K)  & = (1+c)^d  - \frac 2N \sum_{i=1}^N H(\vx_{i}) + \frac{1}{N^2}  \sum_{i,j=1}^N K(\vx_{i},x_{j}),
    \intertext{where}
    h(x) & = \int_{\tOmega} \tK_k(t,x)  \, \tvarrho(t) \, \dif t, \\
    c & = \int_{\tOmega \times \tOmega} \tK(t_k,x_k) \, \tvarrho(t) \tvarrho (x) \, \dif t\dif x = \int_{\tOmega} h(x) \, \tvarrho(x) \, \dif x, \\
    H(\vx) &= \prod_{k=1}^d [1+ h(x_{k})].
\end{align}
We note that an evaluation of $h(x)$ and $\tK(t,x)$ require $\Order(1)$ operations, while an evaluation of $H(\vx)$ and $K(\vt,\vx)$ require $\Order(d)$ operations.  The computation of $D(\Xdes,\rho,K)$ requires $\Order(dN^2)$ operations because of the double sum.
...
\nonumber
& \qquad \qquad +\frac{1}{N^2}\left(2\sum_{i=1}^N  \frac{[\tK(x_{i^*k^*},x_{ik^*})-\tK(x,x_{ik^*})] K(\vx_{i^*},\vx_i)} {1 + \tK(x_{i^*k^*},x_{ik^*})} \right . \\
...
\Delta'(x) = Ah(x)  - \frac{1}{N}\sum_{i=1}^N B_i \tK(x,x_{ik^*}) + C \tK(x,x)
\end{equation}
where
\begin{equation*}
