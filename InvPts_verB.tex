...
\begin{align}
\nonumber
    D^2(\Xdes,\rho,K)  & = c^d  - \frac 2N \sum_{i=1}^N \prod_{k=1}^d h(x_{ik}) + \frac{1}{N^2}  \sum_{i,j=1}^N \prod_{k=1}^d \tK(x_{ik},x_{jk}),
    \intertext{where}
    c & = \int_{\tOmega \times \tOmega} \tK(t_k,x_k) \, \tvarrho(t) \tvarrho (x) \, \dif t\dif x, \\
    h(x) & = \int_{\tOmega} \tK_k(t,x)  \, \tvarrho(t) \, \dif t.
\end{align}
Then the point deletion function becomes 
\begin{align}
\nonumber
\frakd_p(i) 
&= \frac{2}{N}\biggl[ \frac{1}{N-1} \sum_{\substack{j=1 \\ j \ne i}}^N \prod_{k=1}^d h(x_{ik}) - \prod_{k=1}^d h(x_{ik})  \biggr] \\
\nonumber
& \qquad \qquad + \frac{1}{N^2}\biggl[\frac{1-2N}{(N-1)^2}\sum_{\substack{j,m=1 \\ j,m \ne i}}^N \prod_{k=1}^d \tK(x_{jk},x_{mk}) + 2 \sum_{\substack{j=1 \\ j \ne i}}^N\prod_{k=1}^d \tK(x_{ik},x_{jk}) \\
& \qquad \qquad + \prod_{k=1}^d \tK(x_{ik},x_{ik})\biggr].  \label{eq:rowdeletionA}
\end{align}
